...
--
-- Run this AFTER the existing full supabase-setup.md schema you already have.
-- This file contains additive migrations for the Lipila integration and the
-- first-time customer welcome experience and configurable signup bonus.
-- It is safe to run more than once.

begin;

-- Faster lookup when Lipila callbacks arrive with a provider reference.
...
create index if not exists withdrawals_lipila_provider_reference_idx
  on public.withdrawals(provider, provider_reference)
  where provider is not null and provider_reference is not null;

-- The server releases/submits withdrawals after the configured 24-hour hold.
-- Keep pending withdrawals cheap to scan as the wallet grows.
create index if not exists withdrawals_pending_created_idx
  on public.withdrawals(status, created_at)
  where status = 'pending';

-- Payment-link collection callbacks also resolve through the transaction provider reference.
...
-- Provider webhook replay protection already exists in the full schema through
-- UNIQUE(provider,event_id). This index helps operational queries by processing state.
...
-- The admin API exposes notifications, so ensure the permission exists even
-- when the original seed was run before that endpoint was added.

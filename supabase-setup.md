...
Deposits and withdrawals stay `pending` until a trusted provider integration confirms settlement. The frontend cannot credit balances or declare external success. A future Lipila adapter should implement `createCollection`, `checkCollection`, `createWithdrawal`, `checkWithdrawal`, and `handleWebhook`, persist provider references, verify webhook signatures, deduplicate `provider_webhook_events`, then invoke the same database ledger functions.

...
async function processDueWithdrawals() {
  ...
  for (const w of rows || []) {
    try {
      ...
      // Claim the row before calling Lipila so an admin cannot cancel it
      // while the provider request is in flight, and worker ticks cannot
      // submit the same withdrawal twice.
      ...
      if (isProviderSuccess(providerStatus)) {
        ...
        const { error: wu } = await adminClient.from("withdrawals").update({ status: "successful", provider: "lipila", provider_reference: providerReference, updated_at: now }).eq("id", w.id).eq("status", "processing");
        if (wu) throw wu;
        ...
        const now = new Date().toISOString();
        const { error: wu } = await adminClient.from("withdrawals").update({ status: "processing", provider: "lipila", provider_reference: providerReference, updated_at: now }).eq("id", w.id).eq("status", "processing");
        if (wu) throw wu;
      ...
      // A transport/server error means Lipila may not have accepted the
      // request. Return the row to pending so it can be retried; explicit
      // provider rejection still releases the reserved funds.
      if (e.status && e.status < 500) {
        await failWithdrawal(w, e.message);
      } else {
        await adminClient.from("withdrawals").update({ status: "pending", updated_at: new Date().toISOString() }).eq("id", w.id).eq("status", "processing").is("provider_reference", null);
        await adminClient.from("transactions").update({ status: "pending", updated_at: new Date().toISOString() }).eq("id", w.transaction.id).eq("status", "processing");
      }
    }
  }
}
...
app.post("/api/withdrawals", requireAuth, async (req, res, next) => {
  try {
    ...
    if (le) throw le;
    await createNotification(req.user.id, "withdrawal_scheduled", "Withdrawal scheduled", `Your withdrawal will be sent for processing after 24 hours.`, { withdrawal_id: withdrawal.id, available_at: new Date(Date.now() + withdrawalDelayMs).toISOString() });

...
async function processDueWithdrawals() {
  ...
  for (const w of rows || []) {
    try {
      ...
      const ref = w.transaction?.reference || `WD-${w.id}`;
      ...
      await adminClient.from("withdrawals").update({ status: isProviderSuccess(providerStatus) ? "successful" : isProviderFailure(providerStatus) ? "failed" : "processing", provider: "lipila", provider_reference: providerReference, updated_at: new Date().toISOString() }).eq("id", w.id).eq("status", "pending");
      await adminClient.from("transactions").update({ status: isProviderSuccess(providerStatus) ? "successful" : isProviderFailure(providerStatus) ? "failed" : "processing", provider: "lipila", provider_reference: providerReference, completed_at: isProviderSuccess(providerStatus) ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq("id", w.transaction.id);
      if (isProviderSuccess(providerStatus)) await settleSuccessfulWithdrawal(w);
      else await createNotification(w.user_id, "withdrawal_processing", "Withdrawal processing", "Your 24-hour withdrawal hold has completed and Lipila is processing the payout.", { withdrawal_id: w.id, provider_reference: providerReference });
    } catch (e) {
...
app.post("/api/deposits", requireAuth, async (req, res, next) => {
  try {
    ...
    const referenceId = `ZMDEP-${crypto.randomUUID()}`;
    ...
    await db(req).from("deposits").update({ provider_reference: providerReference, status: isProviderSuccess(providerStatus) ? "successful" : isProviderFailure(providerStatus) ? "failed" : "pending", updated_at: new Date().toISOString() }).eq("id", deposit.id);
    await db(req).from("transactions").update({ provider_reference: providerReference, status: isProviderSuccess(providerStatus) ? "successful" : isProviderFailure(providerStatus) ? "failed" : "pending", completed_at: isProviderSuccess(providerStatus) ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq("id", tx.id);
...
app.post("/api/payment-links/:code/pay", async (req, res, next) => {
  try {
    ...
    const referenceId = `ZMLINK-${crypto.randomUUID()}`;
    ...
    const providerReference = provider?.referenceId || provider?.identifier || referenceId;
    await adminClient.from("payment_link_payments").update({ provider_reference: providerReference, status: isProviderSuccess(provider?.status) ? "successful" : isProviderFailure(provider?.status) ? "failed" : "pending" }).eq("id", payment.id);
    await adminClient.from("transactions").update({ provider_reference: providerReference, status: isProviderSuccess(provider?.status) ? "successful" : isProviderFailure(provider?.status) ? "failed" : "pending", completed_at: isProviderSuccess(provider?.status) ? new Date().toISOString() : null }).eq("id", tx.id);

...
async function applyLipilaCollectionCallback(payload, webhookEventId) {
  ...
  const providerReference = payload.identifier || payload.externalId || payload.referenceId || referenceId;
...
app.post("/api/deposits", requireAuth, async (req, res, next) => {
  try {
    ...
    const { data: deposit, error: de } = await db(req).from("deposits").insert({ user_id: req.user.id, amount: input.amount, currency: input.currency, method: input.method, status: "processing", provider: "lipila", description: input.description || null, idempotency_key: key }).select().single();
    if (de) throw de;
    const { data: tx, error: te } = await db(req).from("transactions").insert({ reference: referenceId, user_id: req.user.id, wallet_id: (await userOperationalState(req)).wallet.id, type: "deposit", status: "processing", amount: input.amount, fee: 0, net_amount: input.amount, currency: input.currency, description: input.description || "Lipila deposit", provider: "lipila", provider_reference: referenceId, idempotency_key: key, metadata: { deposit_id: deposit.id, account_number: accountNumber, lipila_reference_id: referenceId } }).select().single();
    if (te) throw te;
    let provider;
    try {
      ...
      await db(req).from("deposits").update({
        status: "failed",
        updated_at: failedAt
      }).eq("id", deposit.id);
      await db(req).from("transactions").update({
        status: "failed",
    ...
    if (isProviderSuccess(providerStatus)) {
      ...
      await applyLipilaCollectionCallback({ referenceId, identifier: providerReference, status: "Failed", amount: input.amount, currency: input.currency, type: "Collection", message: provider?.message || "Lipila could not start the deposit." }, `sync-${referenceId}`);
    } else {
      await db(req).from("deposits").update({ status: "pending", updated_at: new Date().toISOString() }).eq("id", deposit.id);
      await db(req).from("transactions").update({ status: "pending", updated_at: new Date().toISOString() }).eq("id", tx.id);
      await createNotification(req.user.id, "deposit_pending", "Deposit pending", "Approve the secure mobile-money payment request to complete your deposit.", { transaction_id: tx.id, provider_reference: providerReference });
    }
    ...
    ok(res, { deposit: currentDeposit || deposit, transaction: currentTransaction || tx, provider }, 201);
  } catch (e) { next(e); }
});
...
app.get("/api/deposits/:id/status", requireAuth, async (req, res, next) => {
  try {
    ...
    if (["successful", "failed", "cancelled", "reversed"].includes(transaction.status)) {
      return ok(res, { deposit, transaction, provider: null });
    }
    ...
    await db(req).from("deposits")
      .update({ provider_reference: providerReference, updated_at: new Date().toISOString() })
      .eq("id", deposit.id);
    ...
    return ok(res, { deposit: currentDeposit, transaction: currentTransaction, provider });
  } catch (e) { next(e); }
});
...
app.get("/api/admin/withdrawals", requireAdmin, async(req,res,next)=>{
  try {
    ...
    const txResult=userIds.length
      ? await db(req).from("transactions").select("id,user_id,reference,status,provider_reference,metadata").eq("type","withdrawal").in("user_id",userIds)
      : {data:[],error:null};
    if(txResult.error)throw txResult.error;
    const profileResult=userIds.length
      ? await db(req).from("profiles").select("id,display_name,first_name,last_name,zedmoney_id").in("id",userIds)
      : {data:[],error:null};
    if(profileResult.error)throw profileResult.error;
    const txByWithdrawal=new Map((txResult.data||[]).map(tx=>[tx.metadata?.withdrawal_id,tx]));
